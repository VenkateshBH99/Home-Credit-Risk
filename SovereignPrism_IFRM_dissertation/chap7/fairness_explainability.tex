...
\begin{itemize}
    ...
    \item \textbf{Equalized Odds (Recall Disparity):} Checks whether the model is equally accurate at catching actual defaults across groups~\cite{hardt2016}. The equalized odds difference is $|\acs{TPR}_\text{male} - \acs{TPR}_\text{female}|$. A model satisfying equalized odds makes errors at equal rates across groups.
\end{itemize}
...
\ProjectFigure[!htbp][width=0.85\textwidth]{notebook/final_traditional_visualize_fairness_metrics_acr_16.png}{Traditional Fairness Metrics}{Detailed fairness metrics for the traditional dataset across all models, showing selection rates, \acs{TPR}, and \acs{FPR} by gender group.}{fig:fairness_traditional}
...
\begin{enumerate}
    ...
    \item \textbf{\ac{DI} ratios remain $\geq 0.80$} across all models, meeting the four-fifths rule threshold commonly used in regulatory compliance assessments.
\end{enumerate}
...
\textbf{Design note on future mitigation:} the team also reviewed \ac{AIF360}'s Adversarial Debiasing workflow as a possible in-processing mitigation, especially for scenarios where fairness constraints should be learned during training rather than imposed after prediction. It was not executed in the final notebook pipeline, which relies on Fairlearn's \texttt{ThresholdOptimizer}, so it is documented here as a future-work option rather than an implemented experiment~\cite{aif360,adversarial_debiasing}.
...
\begin{itemize}
    ...
    \item \textbf{Anonymisation:} $k$-anonymity ($k \geq 5$) for demographic data and differential privacy ($\epsilon = 1.0$) for spending data, ensuring that individual records cannot be re-identified from aggregated outputs.
    ...
    \item \textbf{Data Residency:} In-country storage compliant with PDPA (Thailand), PDPA (Singapore), and DPA (Philippines), ensuring that applicant data does not cross borders without explicit consent.
    \item \textbf{Right to Erasure:} Applicants can revoke data usage at any time. Upon request, their data is removed from training sets and the model is queued for quarterly retraining to eliminate residual influence.
\end{itemize}
...
\begin{tabular}{L{3.3cm}L{1.8cm}L{1.8cm}L{7.2cm}}
\toprule
