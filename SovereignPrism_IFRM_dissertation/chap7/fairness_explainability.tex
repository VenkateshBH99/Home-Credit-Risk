...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - logistic regression - traditional.png}
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - logistic regression - combined.png}
    \caption{SHAP feature importance for Logistic Regression: Traditional dataset (left) vs Combined dataset (right). The traditional model relies heavily on employment duration and income type, while the combined model shifts toward external source scores.}
    \label{fig:shap_lr}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_bar_shows_fe_3.png}
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_beeswarm_sho_4.png}
    \caption{SHAP bar plot (left) and beeswarm plot (right) for Logistic Regression on the traditional dataset. The beeswarm plot shows the direction of each feature's effect: red dots (high feature values) on the right indicate that higher values push predictions toward default.}
    \label{fig:shap_lr_detailed}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - random forest - traditional.png}
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - random forest - combined.png}
    \caption{SHAP feature importance for Random Forest: Traditional (left) vs Combined (right). External scores dominate in both datasets, with other features contributing substantially less.}
    \label{fig:shap_rf}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_bar_shows_fe_6.png}
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_beeswarm_sho_7.png}
    \caption{SHAP bar plot (left) and beeswarm plot (right) for Random Forest on the traditional dataset. The concentration of importance in EXT\_SOURCE variables is even more pronounced than for Logistic Regression.}
    \label{fig:shap_rf_detailed}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - xgboost - traditional.png}
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - xgboost - combined.png}
    \caption{SHAP feature importance for XGBoost: Traditional (left) vs Combined (right). The combined model incorporates a wider variety of engineered features, spreading predictive influence across more diverse financial behaviours.}
    \label{fig:shap_xgb}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_bar_shows_fe_10.png}
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_beeswarm_sho_11.png}
    \caption{SHAP bar plot (left) and beeswarm plot (right) for XGBoost on the traditional dataset. XGBoost's sequential boosting creates sharper feature importance gradients than Random Forest's bagging.}
    \label{fig:shap_xgb_detailed}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - LightGBM - traditional.png}
    \includegraphics[width=0.45\textwidth]{pptx/SHAP explainability - LightGBM - combined.png}
    \caption{SHAP feature importance for LightGBM: Traditional (left) vs Combined (right). The combined model leverages aggregated data from previous applications and instalment payment history, producing the most diversified feature importance profile.}
    \label{fig:shap_lgbm}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_bar_shows_fe_14.png}
    \includegraphics[width=0.45\textwidth]{notebook/final_traditional_shap_summary_plot_beeswarm_sho_15.png}
    \caption{SHAP bar plot (left) and beeswarm plot (right) for LightGBM on the traditional dataset. The beeswarm reveals clear directional effects: higher EXT\_SOURCE values push toward non-default (left), while younger age pushes toward default (right).}
    \label{fig:shap_lgbm_detailed}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{venky/shap_summary_plot_risk_drivers.png}
    \caption{SHAP summary plot showing the key risk drivers across the champion model. The colour gradient (red = high, blue = low feature value) reveals the directional effect of each feature on default prediction.}
    \label{fig:shap_summary_risk}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{pptx/fairness evaluation by gender.png}
    \caption{Fairness evaluation by gender across all four model families. Demographic parity disparity varies by model, while equalized odds disparity is uniform---indicating that bias originates from the data, not the algorithm.}
    \label{fig:fairness_gender}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{notebook/final_traditional_visualize_fairness_metrics_acr_16.png}
    \caption{Detailed fairness metrics for the traditional dataset across all models, showing selection rates, true positive rates, and false positive rates by gender group.}
    \label{fig:fairness_traditional}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{pptx/gender bias analysis.png}
    \caption{Selection rates by gender across all models. The XNA group (non-disclosed gender) reveals a critical failure mode for Logistic Regression, which predicts exactly 0 for this group.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{venky/fairness_metric_demographic_parity.png}
    \caption{Demographic parity analysis showing selection rates before and after ThresholdOptimizer application. The post-mitigation selection rates are substantially more balanced across gender groups.}
    \label{fig:fairness_dp}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{venky/fairness_metric_equalized_odds.png}
    \caption{Equalized odds analysis showing true positive and false positive rate parity across gender groups. The uniformity of equalized odds across models confirms that bias is data-driven rather than algorithm-driven.}
    \label{fig:fairness_eo}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{notebook/home_credit_modeling_combined_psi_distribution_visualization_28.png}
    \caption{PSI distribution visualisation comparing score distributions between training and validation populations. The PSI value confirms acceptable population stability.}
    \label{fig:psi}
\end{figure}
