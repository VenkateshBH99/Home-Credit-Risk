...
\begin{tabular}{L{3.2cm}L{6.2cm}L{5.3cm}}
\toprule
\textbf{Feature} & \textbf{Formula} & \textbf{Meaning} \\
\midrule
\CodeCell{cc_avg_utilization} & \CodeCell{mean(AMT_BALANCE / AMT_CREDIT_LIMIT_ACTUAL) per applicant} & Average credit card usage relative to the available limit \\
\CodeCell{cc_max_utilization} & \CodeCell{max(AMT_BALANCE / AMT_CREDIT_LIMIT_ACTUAL) per applicant} & Peak utilisation, indicating how close the customer came to maxing out the card \\
\CodeCell{cc_avg_dpd} & \CodeCell{mean(SK_DPD) per applicant} & Chronic lateness signal measured as average days past due \\
\CodeCell{cc_max_dpd} & \CodeCell{max(SK_DPD) per applicant} & Worst observed late-payment episode \\
\CodeCell{cc_late_count} & \CodeCell{sum(SK_DPD > 0) per applicant} & Number of months with any late payment \\
\CodeCell{cc_payment_ratio} & \CodeCell{mean(AMT_PAYMENT_CURRENT / AMT_INST_MIN_REGULARITY) per applicant} & How much the customer pays relative to the minimum due \\
\CodeCell{cc_drawing_ratio} & \CodeCell{mean(AMT_DRAWINGS_ATM_CURRENT / AMT_CREDIT_LIMIT_ACTUAL) per applicant} & ATM cash withdrawal intensity relative to the card limit \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Traditional versus Combined Data Strategy}
\label{tab:trad_vs_alt}
\renewcommand{\arraystretch}{1.18}
\begin{tabular}{p{3cm}p{5.3cm}p{5.3cm}}
\toprule
\textbf{Dimension} & \textbf{Traditional (Application-Only)} & \textbf{Combined (Traditional + Alternative)} \\
\midrule
Primary source & \CodeCell{application_train.csv} only & All 8 interlinked tables aggregated to applicant level \\
Feature count & Approximately 228 engineered features before downstream filtering & Approximately 205 aggregated behavioural and bureau features before downstream filtering \\
Signal type & Static snapshot of who the applicant is at the time of application & Dynamic view of how the applicant behaves with credit over time \\
Strength & Strong baseline for origination decisioning and scorecard interpretability & Richer behavioural context for thin-file applicants and second-order risk patterns \\
Limitation & No repayment history, cross-product exposure, or monthly payment-pattern signal & More complex aggregation logic and higher inter-feature correlation \\
Business reading & Best when bureau and demographic information already exist & Best when behavioural footprints are needed to compensate for sparse bureau history \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{p{2.7cm}p{2.9cm}p{3.4cm}p{3.9cm}}
\toprule
\textbf{Alt-Data Proxy} & \textbf{Real-World Equivalent} & \textbf{What It Captures} & \textbf{How It Helps Thin-File Applicants} \\
\midrule
\CodeCell{EXT_SOURCE_1/2/3} & External model scores & Non-bureau risk signals & Risk signal even with zero bureau records \\
...
Document flags & \ac{KYC}/identity depth & Identity verification willingness & Correlates with repayment intent \\
...
\textbf{The design rationale also aligns with DBS's public framing that credit scores depend materially on financial behaviour history, especially payment behaviour and credit utilisation patterns.}~\cite{dbs_credit_score} That external industry framing maps closely onto this project's decision to aggregate instalment timing, \ac{DPD}, balance utilisation, and previous-application histories instead of treating the application form as a sufficient representation of borrower risk.
...
\ProjectFigure[!htbp][width=0.85\textwidth]{discussion/Credit_Card_Balance_correlation_matrix_key.png}{Feature Correlation Matrix}{Correlation matrix of key features showing multicollinearity patterns. Notable pairs include AMT\_CREDIT/AMT\_GOODS\_PRICE ($r = 0.987$) and AMT\_CREDIT/AMT\_ANNUITY ($r = 0.770$).}{fig:correlation_matrix}
...
The scree plot reveals that PC1 alone covers approximately 17.5\% of variance, but 51 components are required to reach 90\% cumulative coverage. This indicates \textit{no strong latent structure}---there is one dominant signal (likely driven by the EXT\_SOURCE variables) but the remaining information is spread diffusely across many feature clusters. The earlier slide artifact previously placed here was in fact a bureau-credit exposure histogram, so it has been relocated to the bureau EDA section (Chapter~\ref{subsec:bureau_eda}) where it belongs.
